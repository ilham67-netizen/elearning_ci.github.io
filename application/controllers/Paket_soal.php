 <?php
    ...
    class Paket_soal extends CI_Controller
    {
        function __construct()
        {
            ...
            check_not_login();
            check_admin();
            $this->load->model(['m_soal', 'm_tugas', 'm_matkul', 'm_jadwal', 'm_fakultas']);
        ...
        public function index()
        {
            ...
            $this->template->load('template', 'paket_soal/lihat.php', $data);
        }
        public function lihat_paket($kd_prodi)
        {
            ...
            $paket = $this->m_soal->get5($kd_prodi)->result();
            $data = array(
                'title' => "Paket Soal",
                'row' => $paket,
                'kd_auto' => $kode_hasil,
            ...
            $this->template->load('template', 'paket_soal/lihat_paket', $data);
        }
        public function lihat_soal($kd_prodi, $kd_paket)
        {
            ...
            $this->template->load('template', 'paket_soal/lihat_soal', $data);
        }
        public function tambah_soal($kd_prodi, $kd_paket)
        {
            ...
            $hitung = $this->m_soal->get3($kd_paket);
            if ($hitung->num_rows() > 0) {
                ...
                if ($jenis->jenis_soal == 1) {
                    $this->template->load('template', 'paket_soal/tambah_essay', $data);
                } elseif ($jenis->jenis_soal == 2) {
                    $this->template->load('template', 'paket_soal/tambah_pilgan', $data);
                } elseif ($jenis->jenis_soal == 4) {
                    $this->template->load('template', 'paket_soal/tambah_pilgansay', $data);
                }
            }
        }
        public function edit_paket($kd_prodi, $kd_paket)
        {
            $hitung = $this->m_soal->get3($kd_paket);
            if ($hitung->num_rows() > 0) {
                $paket = $hitung->row();
                $matkul = $this->m_matkul->get3($kd_prodi, $paket->kd_matkul)->result();
                $data = array(
                    'title' => "Edit Paket Ujian",
                    'row' => $paket,
                    'matkul' => $matkul
                );
                $this->template->load('template', 'paket_soal/edit_paket', $data);
            } else {
                echo "<script>
 			alert('Data tidak ditemukan');";
                echo "window.location='" . site_url('paket_soal/lihat_paket/' . $kd_prodi) . "';
 			</script>";
            }
        }
        public function process($kd_prodi)
        {
            ...
            redirect(base_url('paket_soal/lihat_paket/' . $kd_prodi));
        }
        public function proses_soal($kd_prodi, $kd_paket)
        {
            ...
            redirect(base_url('paket_soal/lihat_soal/' . $kd_prodi . '/' . $kd_paket));
        }
        public function proses_essay($kd_prodi, $kd_paket)
        {
            $post = $this->input->post();
            if (isset($post['add'])) {
                // mengambil data kode
        ...
        //         }
        //     }
        //     redirect(base_url('paket_soal/lihat_soal/' . $post['kd_paket']));
        // }
        ...
        public function del_soal($kd_prodi, $kd_paket, $kd_soal)
        {
            if (isset($kd_soal)) {
